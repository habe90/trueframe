...
<div class="mb-4">
        ...
        <input type="text" name="title" id="title" class="input" value="{{ old('title', $Product->title) }}">
        @if(errors('title'))<p class="text-red-500 text-xs mt-1">{{ errors('title')[0] }}</p>@endif
      </div>
<div class="mb-4">
        <label for="price" class="block text-sm font-medium text-gray-700 mb-1">Price</label>
        <input type="number" step="0.01" name="price" id="price" class="input" value="{{ old('price', $Product->price) }}">
...
<div class="mb-4">
        ...
        <input type="checkbox" name="active" id="active" class="h-4 w-4 text-gray-600 border-gray-300 rounded" {{ old('active', $Product->active) ? 'checked' : '' }}>
        @if(errors('active'))<p class="text-red-500 text-xs mt-1">{{ errors('active')[0] }}</p>@endif
      </div>

      <div class="flex justify-end mt-6">
        <button type="submit" class="btn bg-blue-600 hover:bg-blue-700">Update</button>
